...
class QRCodeScanActivity : CaptureActivity() {
    companion object {
        private const val REQUEST_FOR_SCAN = 1000000
        fun start(activity: Activity) {
            Intent(activity, QRCodeScanActivity::class.java).also {
                activity.startActivityForResult(it, REQUEST_FOR_SCAN)
            }
        }

        fun parseResult(requestCode: Int, resultCode: Int, data: Intent?): String {
            return if (requestCode == REQUEST_FOR_SCAN && resultCode == RESULT_OK) {
